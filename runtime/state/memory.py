...
def growth_memory_from_events(events: list[dict]) -> dict[str, Any]:
    ...
    Each entry: {arc_id, topic_id, verdict, pattern_id, supporting_event_ids}
    — verdict ∈ confirmed / refuted / inconclusive (ADR-007 §1). Provenance
    is mandatory (ADR-012 §3): every memory cites the events that produced it."""
    topic_by_arc: dict[str, tuple[str, str]] = {}
    pattern_by_arc: dict[str, str] = {}
    ...
    for ev in events:
        ...
        if etype == "mission.activated":
            ...
            pattern = payload.get("pattern_id") or arc.get("pattern_id")
            if pattern:
                pattern_by_arc[payload["arc_id"]] = pattern
            ...
            closed.append({
                ...
                "pattern_id": pattern_by_arc.get(arc_id),
