...
## 待用户确认的开放问题
1. **Q24 未回答**：日常陪伴启动方式（推荐 C：豆豆兔主动邀请 + 孩子随时召唤）。
2. 蓝图建议 MVP 3 个 Agent（Partner/Adventure/Memory）拆分 — 本计划 Phase 1-5 先保持单进程模块，Agent 边界以模块接口预留；是否接受？
3. 客户端（Phase 6 之后）是否另起仓库？
